...
//	EXPECT_EQ( S_OK, objectInRootFolder->SetName("allAscendants") );
//	EXPECT_EQ( S_OK, objectInRootFolder->SetExpression("let kinds = Set{ \"Model\", \"Atom\", \"Set\", \"Reference\", \"Connection\",\"FCO\" } in \nif ( kinds.includes( self.kindName )) then \nself.ascendantEndsRecursive( Set{}, inhType ) \nelse \nSet{ } \nendif") );
	delete allAscendants;

	MetaConstraint *allAspects;
	EXPECT_EQ( S_OK, rootFolder->CreateConstraint(allAspects) );
//	EXPECT_EQ( S_OK, objectInRootFolder->SetName("allAspects") );
//	EXPECT_EQ( S_OK, objectInRootFolder->SetExpression("method;gme::FCO;;ocl::Set; \nself.allAscendants( 1 ) -> iterate( fco : gme::FCO ; accu = Set{} | accu + fco.memberOfSets() )") );
	delete allAspects;
